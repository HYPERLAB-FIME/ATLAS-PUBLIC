...
| 15  | [Why is trust so important at this stage of agentic commerce? What about long term?](#15-why-is-trust-so-important-at-this-stage-of-agentic-commerce-what-about-long-term)                                                     |
| 16  | [What protocols help merchants distinguish a legitimate agent from a scam bot or scraper?](#16-what-protocols-help-merchants-distinguish-a-legitimate-agent-from-a-scam-bot-or-scraper)                                        |
| 17  | [What does the audit trail for an agentic purchase look like? What proves consumer intent vs agent execution?](#17-what-does-the-audit-trail-for-an-agentic-purchase-look-like-what-proves-consumer-intent-vs-agent-execution) |
| 18  | [Does an agent carry a portable credential proving its authority from merchant to merchant?](#18-does-an-agent-carry-a-portable-credential-proving-its-authority-from-merchant-to-merchant)                                    |
| 19  | [If an agent buys the wrong item or exceeds budget, who bears the loss?](#19-if-an-agent-buys-the-wrong-item-or-exceeds-budget-who-bears-the-loss)                                                                             |
| 20  | [What is the risk of fragmentation from different agentic commerce protocols?](#20-what-is-the-risk-of-fragmentation-from-different-agentic-commerce-protocols)                                                                |
| 21  | [Is merchant willingness about technical readiness or genuine acceptance of the channel?](#21-is-merchant-willingness-about-technical-readiness-or-genuine-acceptance-of-the-channel)                                          |
| 22  | [How far out is workable trust infrastructure, and what is the biggest near-term risk?](#22-how-far-out-is-workable-trust-infrastructure-and-what-is-the-biggest-near-term-risk)                                               |
...
| **Regulatory / rulebook compliance at transaction time** (payment-method choice; **IFR** as one EU example) | Many rules assume a **human** chose the merchant, the product, and the **payment instrument / path**. An agent can pick a card, wallet, or rail that changes **cost, routing, or legal treatment** without that choice being transparent to the user or the scheme. **Example — IFR** (EU Interchange Fee Regulation): it caps certain interchange fees and sits in a broader EU push for clear payment-method choice and cost fairness — so if an agent steers the instrument (e.g. toward a higher-cost card product or a path the user did not knowingly select), interchange and related cost treatment can become hard to justify under those rules. The same pattern applies beyond IFR: network rulebooks, restricted goods, age limits, geo/sanctions, SCA expectations, and other obligations that must hold **at transaction time**, not only at agent onboarding. Rail choice may also come **after** the agent has already shaped the cart |
| **Unclear liability**                                                                                       | When an agent-led purchase fails policy or is disputed, schemes need a fair way to allocate responsibility. ATLAS helps the **liability model** by providing an **independent, signed, redeemable assessment** of what the agent did (and against which rulebook) — evidence schemes, issuers, and merchants can use to **allocate responsibility or evolve liability rules later**, based on verified behaviour rather than each party’s unverifiable claim                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
**Two signal legs (complementary, not a hard timeline split):**

- **[ATLAS](https://hyperlab-fime.github.io/ATLAS-PUBLIC/atlas-protocol-specification-0.1draft.html)** — independent assessment of **runtime behaviour** along the path from intent capture → discovery → ranking → cart: compliance, intent fidelity, KYA standing, dispute support, oversight.
- **[AP2](https://ap2-protocol.org/ap2/specification/) / [VI**](https://verifiableintent.dev/spec/) — **authorization and binding artefacts** (consent, mandate, cart integrity, payment instrument, audit trail). Intent/mandate objects can appear earlier in the journey, but they are typically **finalized and relied on at checkout** — and they do **not** by themselves re-audit *how* the agent discovered, ranked, and built that cart.

**One-liner:** KYA-OS = ID badge; AP2/VI = permission and binding to pay; ATLAS = missing independent signals about behaviour, compliance, and oversight *before* those artefacts are treated as ground truth.
...
13. **Early trust foundation** that scales from human-in-the-loop flows toward delegation, micropayments, and machine-to-machine commerce.

---



## 15. Why is trust so important at this stage of agentic commerce? What about long term?

Technology matters only insofar as it *sustains* trust: merchants, issuers, networks, and consumers need evidence they can use to accept an agent, allow a purchase, and resolve disputes later — not another stack for its own sake.

At this stage the ecosystem is still emerging, and most flows still keep a human in the loop. Models, tools, and threat surfaces are evolving quickly. Classic CNP signals (built over decades) do not fully transfer when an agent interprets intent, picks merchants, and builds the cart. So the industry needs a shared foundation of **trust signals** from day one — compact, independent, and portable across parties — instead of bolting oversight on after losses accumulate.

Longer term, the same signal layer unlocks the next wave: fuller **delegation** (human no longer in the loop), **micro-payments** (e.g. HTTP 402-style rails), and **machine-to-machine** B2B commerce, where continuous oversight without a human gatekeeper is non-negotiable. ATLAS is designed as that shared assessment layer — restoring missing pre-transaction signals beside identity (KYA) and checkout binding (AP2/VI).

---



## 16. What protocols help merchants distinguish a legitimate agent from a scam bot or scraper?

Merchants need to separate two worlds. **Browser-based** commerce spent years stopping malicious bots from acting like humans; that stack (bot detection and similar controls) now has to evolve to tell *legitimate* agents from *malicious* ones. That requires **verifiable agent identity** on the request — approaches such as **Web Bot Auth** (an IETF draft profile built on **RFC 9421** HTTP Message Signatures) are promising for carrying identity, but they do not by themselves prove the agent’s **allowed scope of behaviour**. Network programs such as Visa Trusted Agent / Mastercard Agent Pay (and related agentic commerce stacks) similarly strengthen recognition and scoped credentials; they still leave a gap on independent runtime behaviour. In parallel, a **dedicated agentic** stack is emerging that no longer depends on rendering a UI for a human. That stack is better suited to (1) recognise the agent, (2) verify it is authorised for a given action, and (3) leave an **audit trail** — with chained, verifiable evidence across payment method, merchant basket, user consent/authentication, and delegation mandates. Merchant confidence in today’s signals is still **uneven**: identity signals are improving, but **runtime behaviour and scope** remain the weak link without independent assessment — which is what ATLAS Assessors add.

---



## 17. What does the audit trail for an agentic purchase look like? What proves consumer intent vs agent execution?

On emerging standards such as **AP2** and **Verifiable Intent (VI)**, key checkout elements are captured deterministically into verifiable credentials: the user intent, merchant basket, selected payment method, and transaction data, often signed with the user’s device key. That chain is **necessary** evidence for checkout and post-purchase binding. It is not sufficient for the full story: those artefacts are typically collected **after** the agent has already interpreted intent, chosen merchants, and built the cart. A **complementary** chain of verifiable evidence should also be captured **up front**, while the agent is processing the user’s initial intent — covering discovery, ranking, and cart construction — so the audit trail proves both **consumer intent fidelity** and **agent execution**, not only the final checkout object the user confirmed. ATLAS assessment (signed verdict + redeemable report token) is designed to provide that independent pre-transaction record.

---



## 18. Does an agent carry a portable credential proving its authority from merchant to merchant?

Emerging work such as **DIF KYA-OS** aims to carry both **agent identity** and **allowed scope**. The harder operational question is how a merchant verifies identity and scope **before** any purchase — and often **before** any payment rail is even chosen. Cross-repository KYA checks are complex: one can imagine a KYA store and authorised scopes **per payment network**, yet during shopping no rail may be identified yet. Asking every merchant to query every known KYA repository for every requesting agent is impractical. This is where an ATLAS **Assessor** helps: it can perform identity and authorisation checks **across** repositories and return a compact result the merchant can rely on, instead of each merchant implementing full KYA verification itself. Portable authority is **emerging, not fully standardised** end-to-end across merchants and rails.

---



## 19. If an agent buys the wrong item or exceeds budget, who bears the loss?

In today’s emerging models, **merchants** often still absorb much of the chargeback cost. Classic three-party fraud framing (1st-party: cardholder as fraudster; 2nd-party: misuse with consent; 3rd-party: fraudster against the genuine user) does not cleanly cover agent-mediated failure. A useful **proposed framing** — not yet standard industry taxonomy — is a **fourth** case: the **user’s agent acted outside approved behaviour** (or drifted from stated constraints) even though the cardholder and merchant are both “genuine.” Dispute resolution is based on **signals collected from initial intent through delivery**. Richer trust signals can **support** future **liability protection or shift** frameworks for merchants **if** schemes and issuers adopt them — analogous *in spirit* to how CNP evolved with EMV 3-D Secure, but **agent-specific liability rules are not published as settled fact today**. Emerging checkout protocols such as **AP2** and **VI** do reintroduce important transaction-time signals (consent, mandate, cart binding, payment instrument). Even so, some **key signals remain partial or missing** — especially independent evidence of how the agent interpreted intent and built the cart *before* those artefacts were bound. ATLAS is designed to **restore** those missing pre-transaction signals as independent, redeemable records so schemes can evolve liability and dispute rules on fuller evidence, not “he said / agent said.”

---



## 20. What is the risk of fragmentation from different agentic commerce protocols?

The main risk is **lack of interoperability**. If a payment card only worked on a few terminals in one region, people would either carry many cards or abandon cards for something more universal. Agentic commerce faces the same adoption cliff: merchants, agents, and downstream parties cannot afford to implement many incompatible stacks. Securing every proprietary channel is costly and unrealistic; improving security on **one shared mechanism** is. **ATLAS** is intentionally **protocol-agnostic** — flexible while protocols emerge, and compatible later with dedicated standards — so assessment and trust signals can travel with the transaction without locking the ecosystem to one vendor island.

---



## 21. Is merchant willingness about technical readiness or genuine acceptance of the channel?

Merchants (like every other actor) care about **revenue**. The real question is trust, not technology for its own sake — technology **sustains** trust. Do merchants have enough signals to protect revenue and limit chargeback loss? CNP e-commerce spent decades building those signals (e.g. EMV 3-D Secure). Agentic commerce needs the same: signals that support regulation, dispute mechanisms, and authentication of both user and agent — plus **ongoing agent assessment**, because agents can act autonomously and must be overseen. That is what ATLAS targets. Consumers will accept the channel when they trust that constraints (budget, preferences, safety) survive agent interpretation — not only that a confirmation screen looked plausible.

Merchant willingness is not primarily a tech-readiness checkbox. It is about **sufficient trust** and **reassurance of business revenue** — with **acceptable fraud mitigation** — so accepting agent-initiated transactions feels commercially safe.

---



## 22. How far out is workable trust infrastructure, and what is the biggest near-term risk?

The ecosystem is multi-speed. In some regions it is **market-driven**: deploy the commerce stack, mitigate fraud against revenue, harden security over time. In others it is **regulation-driven**: guardrails and compliance must exist before volume, and regulators must also clarify new rules — so adoption is slower. Many markets are a mix. The **common denominator risk is revenue loss** — from chargebacks, fraud, or penalties for non-compliance. Early commercial agentic flows **may** appear around major retail peaks **in some early / market-driven regions**; elsewhere workable trust infrastructure and oversight will take longer — treat any single calendar as a scenario, not a guarantee.

Near term, the binding constraint is usually **insufficient trust signals and dispute readiness**, not a single binary of “fraud vs backlash vs regulation.” The sharper near-term risk is **exposing consumers to agentic-related threats without sufficient mechanisms to ensure trust** — intent drift, unauthorised agent behaviour, opaque disputes. Damage would not stop with the shopper: **consumers, merchants, and payment brands** would all feel material harm (losses, chargebacks, reputational and regulatory hit) if volume scales ahead of workable oversight.
...
- Combined ATLAS + AP2/VI signal framework (complementary behaviour assessment vs authorization/binding artefacts)
